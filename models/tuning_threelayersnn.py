...
def train_for_params(time_step=1e-2, nb_hidden1=15, nb_hidden2=15, tau_syn=10e-3, tau_mem=10e-3, learning_rate=1e-3,
    ...
    ThreeLayerSNN = SNN(hyperparams=hyperparams, nb_inputs=nb_inputs, nb_outputs=nb_outputs, nb_layers=nb_layers,
                        max_time=max_time)
    loss_hist_train = ThreeLayerSNN.train(X_train, y_train, path=path, early_stopping=False)
    valloss = ThreeLayerSNN.evaluate_loss(X_val, y_val)
...
path = "/local/work/enguyen/tuning/results_threelayersnn/"
