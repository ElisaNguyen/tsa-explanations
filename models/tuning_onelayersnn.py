...
def train_for_params(time_step=1e-2, tau_syn=10e-3, tau_mem=10e-3, learning_rate=1e-3, batch_size=256):
    ...
    hyperparams = {'batch_size': batch_size,
                   ...
                   'tau_syn': tau_syn,
                   'time_step': time_step}
    OneLayerSNN = SNN(hyperparams=hyperparams, nb_inputs=nb_inputs, nb_outputs=nb_outputs, nb_layers=nb_layers,
                      max_time=max_time)
    loss_hist_train = OneLayerSNN.train(X_train, y_train, path=path, early_stopping=False)
    valloss = OneLayerSNN.evaluate_loss(X_val, y_val)
...
path = "/local/work/enguyen/tuning/results_onelayersnn/"
